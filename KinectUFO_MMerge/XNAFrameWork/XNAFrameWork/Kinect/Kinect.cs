        ...
        */
        ...
        public void UpdateSkeletonFrame(object sender, SkeletonFrameReadyEventArgs e)
        {
            ...
            */
            ...
            //No skeletons detected from kinect
            if (skeletonFrame == null)
            {
                trackingID = -1; //set tracking id to -1
        ...
        #region Tracking Specific Person

        public bool Tracking;
        private int trackingID = -1;

        public void Recalib(Skeleton[] skeletonData)
        {
            if (this.kinect != null && this.kinect.SkeletonStream != null)
            {
                int checkID = 0;

                //Check skeleton array
                foreach (Skeleton skel in skeletonData.Where(s => s.TrackingState != SkeletonTrackingState.NotTracked))
                {
                    this.kinect.SkeletonStream.ChooseSkeletons(skel.TrackingId);
                    //Check if they are doing T-pose

                    //Get height of shoulder center
                    float ShoulderY = skel.Joints[JointType.ShoulderCenter].Position.Y;

                    //Get height of left hand & elbow
                    float LeftHandY = skel.Joints[JointType.HandLeft].Position.Y;
                    float LeftElbowY = skel.Joints[JointType.ElbowLeft].Position.Y;

                    //Get height of right hand & elbow
                    float RightHandY = skel.Joints[JointType.HandRight].Position.Y;
                    float RightElbowY = skel.Joints[JointType.ElbowRight].Position.Y;

                    //Difference threshold
                    float DIFF = 0.3f;

                    //Get difference between height of shoulder and height of left hand & elbow
                    float DiffLHandY = ShoulderY - LeftHandY;
                    float DiffLElbowY = ShoulderY - LeftElbowY;

                    if (DiffLHandY < 0) DiffLHandY *= -1;
                    if (DiffLElbowY < 0) DiffLElbowY *= -1;

                    //Get difference between height of shoulder and height of right hand & elbow
                    float DiffRHandY = ShoulderY - RightHandY;
                    float DiffRElbowY = ShoulderY - RightElbowY;

                    if (DiffRHandY < 0) DiffRHandY *= -1;
                    if (DiffRElbowY < 0) DiffRElbowY *= -1;
                    ...
                    //Diff between distance from kinect of left & right hand
                    float DiffZ = skel.Joints[JointType.HandLeft].Position.Z - skel.Joints[JointType.HandRight].Position.Z;

                    float DIFF_Z = 0.2f;

                    //Diff between distance from kinect of shoulder & left hand
                    float DiffZ_S = skel.Joints[JointType.HandLeft].Position.Z - skel.Joints[JointType.ShoulderCenter].Position.Z;
                    float DIFF_Z_S = 0.3f;

                    if (DiffZ < 0) DiffZ *= -1;
                    if (DiffZ_S < 0) DiffZ_S *= -1;

                    //If skeleton in T-pose 
                    if (DiffLHandY > 0 && DiffLHandY < DIFF && //Left Hand Height
                        DiffLElbowY > 0 && DiffLElbowY < DIFF && //Left Elbow Height
                        DiffRElbowY > 0 && DiffRElbowY < DIFF && //Right Hand Height
                        DiffRHandY > 0 && DiffRElbowY < DIFF && //Right Elbow Height
                        DiffZ > 0 && DiffZ < DIFF_Z &&
                        DiffZ_S > 0 && DiffZ_S < DIFF_Z_S
                        )
                    {
                        checkID = skel.TrackingId; //Get tracking id of this skeleton

                        t_Skeleton = skel;
                    }

                    if (checkID > 0)
                        break; //Don't look for anymore t-pose
                }

                //Set new skeleton to track
                if (checkID > 0)
                {
                    trackingID = checkID;
                    this.kinect.SkeletonStream.ChooseSkeletons(trackingID);
                    Tracking = true;
                }
        ...
        private float DLHY, DLEY, DRHY, DREY;
        public void DrawDebug()
        {
            //DebugText.GetInstance().Printf("Left Hand: " + DLHY.ToString(), new Vector2(1000, 50));
            //DebugText.GetInstance().Printf("Left Elbow: " + DLEY.ToString(), new Vector2(1000, 100));
            //DebugText.GetInstance().Printf("Right Hand: " + DRHY.ToString(), new Vector2(1000, 150));
            //DebugText.GetInstance().Printf("Right Elbow: " + DREY.ToString(), new Vector2(1000, 200));
            //DebugText.GetInstance().Printf("Track ID" + trackingID.ToString(), new Vector2(1000, 350), new Color(0, 0, 0));

            if (skeleton == null) return;

            //DebugText.GetInstance().Printf("Tracking : " + skeleton.TrackingState.ToString(), new Vector2(1000, 400));
            //DLHY = skeleton.Joints[JointType.HandLeft].Position.Y;
            //DLEY = skeleton.Joints[JointType.ElbowLeft].Position.Y;
            //DRHY = skeleton.Joints[JointType.HandRight].Position.Y;
            //DREY = skeleton.Joints[JointType.ElbowRight].Position.Y;

            //DebugText.GetInstance().Printf("Hand Diff X: " + HandX.ToString(), new Vector2(1000, 250));
            //DebugText.GetInstance().Printf("Left Hand X: " + LeftHandX.ToString(), new Vector2(1000, 450));
        }
    }
}
