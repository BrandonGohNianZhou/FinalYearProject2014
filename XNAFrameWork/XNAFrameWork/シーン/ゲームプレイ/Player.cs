...
namespace XNAFrameWork
{
	class Player : IObject
	{
		...
		//--------------------------------------------------------------//
		// 関数名	ControllPlayerWithSkeleton							//
		//	Function name ControllPlayerWithSkeleton
		// 機能		スケルトンデータに連動させてキャラクターを動かす		//
        ...
        // testing
        public void ControllByTilting(GameTime gameTime)
        {
            int speed = 250;
            // moving the player left and right after knowing the maximium space the player has
		...
		public void MovingPlayer()
		{
            ...
            this.TotalDis += Math.Abs(this.vel.Z);
		}
		...
		private void JudgJump()
		{
			...
			if (this.isJump == false && this.jumpFlag == false)
			{
                ...
                if ((Game1.person.GetJointPos(JointType.Spine).Y - Game1.person.GetSecondJointPos(JointType.Spine).Y)
                    >= JUMP_VALUE)
                {
                    if (!jumpFlag2)
                    {
        ...
        {
            // debug duck
            //if (MyKeyboard.IsPress(Keys.M))
            //{
            //    animationPlayer.StartClip(clip);
            //    animationPlayer.currentTimeValue = TimeSpan.FromSeconds(156 / 30);
        ...
        public bool InitStartPos()
        {
            ...
            {
                if (avateerHandler.GetJointRotation(BoneType.ForeArmLeft).X > -0.3 && avateerHandler.GetJointRotation(BoneType.ForeArmRight).X > -0.3) 
                {
        ...
        public void AnimationUpdate(GameTime gameTime)
        {
            ...
            {
                // reach the end of ducking animation
                if (animationPlayer.currentTimeValue >= TimeSpan.FromSeconds(159 / 30))
                {
                    this.isDuck = false;
		...
		//----------------------------------------------------------//
		// 関数名	DrawSkeletonModel								//
		//	Function name DrawSkeletonModel
		// 機能		スケルトンが取得できているときのモデルの描画		//
		...
		//----------------------------------------------------------//
		// 関数名	DrawDefaultModel								//
		//	Function name DrawDefaultModel
		// 機能		スケルトンが取得できていないときのモデルの描画		//
