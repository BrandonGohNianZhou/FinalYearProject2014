...
{
	class StageCollisionPart3 : IStageCollision
    {
        #region PASS_SAFELY
        ...
        BoardTexture[] boardTexture = new BoardTexture[4];

		#endregion

		#region Constructor

		public StageCollisionPart3()
		{
			// Initialization
		...
		public void Update(Vector3 stageCenterPoint)
		{
			...
			this.boardTexture[0].PosZ = stageCenterPoint.Z + 230.0f;
			this.boardTexture[0].scaleX = 60.0f * 0.4f;
            this.boardTexture[0].scaleY = 6.0f * 0.4f;
			this.boardTexture[0].scaleZ = 4.0f;


			// Wall 2
			this.boardTexture[1].PosX = 14.0f;  // 37f
			this.boardTexture[1].PosY = stageCenterPoint.Y + 18.0f; //20
			this.boardTexture[1].PosZ = stageCenterPoint.Z;
            this.boardTexture[1].scaleX = 10.0f * 0.38f;
			this.boardTexture[1].scaleY = 15.0f + 1.0f;
			this.boardTexture[1].scaleZ = 4.0f;


			// Wall 3
			this.boardTexture[2].PosX = 0.0f;
			this.boardTexture[2].PosY = stageCenterPoint.Y + 18.0f; //20
			this.boardTexture[2].PosZ = stageCenterPoint.Z - 270.0f;    //280
            this.boardTexture[2].scaleX = 10.0f * 0.38f;
			this.boardTexture[2].scaleY = 16.0f;
        ...
        public bool Hit(Vector3 playerPos, bool playerDuck)
        {
            ...
            if (!this.passed3)
            {
                if (Collision.PlayerAndTextureCollision(playerPos, boardTexture[2]))
                {
